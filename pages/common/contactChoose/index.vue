...
export default {
  ...
  methods: {
    ...
    confirm() {
	  console.log('转发消息类型', this.type, this.activeTab)
      if (this.activeTab) {
        this.activeTab = 0;
        return;
      }
      ...
      this.getCheckedInfo.map(async (item) => {
        ...
        if (this.type === ContactChooseTypes.ForWard) {
			console.log('转发消息类型122', this.forwardMessage)
        ...
        IMSDK.asyncApi(IMMethods.SendMessage, IMSDK.uuid(), {
          ...
          .catch(({ data, errCode, errMsg }) => {
			console.log('发送转发消息类型fail//////////////, ', errCode,errMsg )
            if (!isInConversation) return;
            this.$store.dispatch("message/updateOneMessage", {
              message: data,
              type: UpdateMessageTypes.KeyWords,
